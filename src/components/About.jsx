import React from 'react';
import profileImage from '../assets/arrimon.jpeg';
import SectionHeader from './SectionHeader';
import GitHubStats from "./GitHubStats";
import TypingText from './TypingText';

const DetailItem = ({ label, value }) => (
  <div className="space-y-1">
    <p className="text-[var(--text-muted)] text-[10px] sm:text-xs font-semibold uppercase tracking-wider">{label}</p>
    <p className="text-[var(--text-primary)] font-medium text-xs sm:text-sm break-words">{value}</p>
  </div>
);

const About = () => {
  return (
    <div className="space-y-8 md:space-y-12 py-20">
      {/* <div className="text-center">
        <h2 className="text-3xl md:text-4xl font-extrabold mb-2 text-[var(--text-primary)]">About Me</h2>
        <div className="w-12 h-1 bg-[var(--accent)] mx-auto rounded-full"></div>
      </div> */}
      <SectionHeader number={1} title="About Me" />

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12 items-center">
        
        {/* Personal Details Card */}
        <div className="lg:col-span-2 bg-[var(--surface)] rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-8 border border-[var(--border)] space-y-6 md:space-y-8">
          <div className="flex items-center gap-4 border-b border-[var(--border)] pb-6">
             <div className="w-14 h-14 md:w-16 md:h-16 rounded-xl md:rounded-2xl overflow-hidden shadow-inner bg-[var(--surface)] flex-shrink-0">
                <img src={profileImage} alt="Rimon" className="w-full h-full object-cover opacity-90" />
             </div>
             <div>
                <h4 className="font-bold text-[var(--text-primary)] text-lg md:text-xl">Abu Rayhan Rimon</h4>
                 <p className="text-[var(--accent)] text-sm font-medium">Full Stack Developer</p>
             </div>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-4">
            <DetailItem label="Full Name" value="Md Abu Rayhan Rimon" />
            <DetailItem label="Email" value={<a href="mailto:dev.rrimon@gmail.com" className="text-[var(--text-primary)] hover:underline break-all">dev.rrimon@gmail.com</a>} />
            <DetailItem label="Nationality" value="Bangladeshi" />
            <DetailItem label="Languages" value="English, Bangla" />
          </div>
        </div>

        {/* Narrative Text */}
        <div className="lg:col-span-3 space-y-5 md:space-y-6 text-[var(--text-muted)] leading-relaxed text-sm sm:text-base md:text-lg">
          <p>
            I am a <span className="text-[var(--accent)] font-bold"><TypingText words={["Full Stack Developer", "React Developer", "Laravel Developer"]} /></span> specializing in building high-performance, scalable web applications that bridge the gap between complex backend architecture and premium user interfaces. With over <span className="text-[var(--text-primary)] font-semibold">1.5 years of experience</span>, I excel at crafting robust systems—ranging from CRM platforms to transaction reconciliation engines—using the <span className="text-[var(--text-primary)] font-semibold">Laravel</span> and <span className="text-[var(--text-primary)] font-semibold">React</span> ecosystem.
          </p>
          
          <p>
            At <span className="text-[var(--text-primary)] font-semibold border-b-2 border-[var(--border)]"><a href="https://www.race.net.bd/" target='_blank' rel="noreferrer">Race Online Ltd.</a></span>, I focus on writing clean, efficient code that directly improves business productivity and system performance. I don't just build features; I architect solutions that are <span className="text-[var(--accent)] font-semibold">secure</span>, <span className="text-[var(--accent)] font-semibold">data-driven</span>, and optimized for a seamless user experience. I am deeply committed to leveraging modern development workflows and <span className="text-[var(--text-primary)] font-semibold">AI-assisted tools</span> to deliver production-grade software with precision.
          </p>
        </div>
      </div>
      {/* GuitHub Stats */}
      {/* <GitHubStats username="arrimon" /> */}
    </div>
  );
};

export default About;